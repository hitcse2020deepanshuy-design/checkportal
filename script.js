...
function login(){
  ...
  if(!USERS[r]||USERS[r].password!==p||USERS[r].dob!==d){ error.textContent="❌ Invalid credentials."; return; }
  error.textContent="";

  const u=USERS[r], total=u.hindu+u.ca+u.desc+u.weeklyH+u.weeklyCA;
  ...
  attemptedEl.textContent=attempted; correctEl.textContent=correct; wrongEl.textContent=wrong; accuracyEl.textContent=acc+"%";

  motivationBox.innerHTML=finalPass ? "🎉 <b>Great attempt!</b> You’ve cleared the cut-off. Keep pushing 🚀"
  ...
  status.textContent=finalPass?"Qualified":"Not Qualified"; status.className=finalPass?"pass":"fail";
